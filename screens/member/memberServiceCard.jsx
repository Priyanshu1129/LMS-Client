...
const ServiceCard = ({service, token})=>{
  ...
  const translateY = slideAnimation.interpolate({
    ...
    <View style={[styles.card]}>
        ...
        <View style={[styles.serviceHeader, {backgroundColor : colors.buttonBackground}]}>
             ...
             <View style={[{display : 'flex', flexDirection : 'row', gap : 4, alignItems : 'center'}]}>
                ...
                <View style={[{width : 12, height : 12, backgroundColor : 'yellow', borderRadius : 4}]}></View>
             </View>
             <TouchableOpacity onPress={toggleOptions} activeOpacity={0.9}>
             <View>
               {!openMenue ? <MaterialCommunityIcons size={25} name={"menu-down"} color = {'white'}/>
                 : <MaterialCommunityIcons size={25} name={"menu-up"} color = {'white'} />}
             </View>
             </TouchableOpacity>
        </View>
        {openMenue && (
        ...
        >
          <View style={styles.userInfoRow}>
             <ServiceDetailsDropdownCard service={service} token = {token}/>
          </View>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
    ...
    title: { textAlign: "center", fontSize: 16, fontWeight: "bold", color : 'white' },
    cardInfo: {
      padding: 20,
      display: "flex",
      gap: 16,
